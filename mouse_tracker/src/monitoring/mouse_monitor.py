...
import pandas as pd
import threading
from datetime import datetime, timedelta  # 添加了timedelta导入
...
class MouseMonitor:
    def __init__(self, analysis_interval=120, output_dir="mouse_data", stop_event=None):
        """
        初始化鼠标监控器
        ...
        # 分析结果文件
        self.analysis_file = os.path.join(self.output_dir, "mouse_performance.csv")
    ...
    def analyze_period(self):
        ...
        with self.lock:
            ...
            # 保留最后5秒的事件以确保连续性
            cutoff_time = datetime.now() - timedelta(seconds=5)  # 修复了timedelta使用
